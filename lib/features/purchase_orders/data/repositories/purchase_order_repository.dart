...
class PurchaseOrderRepository {
  ...
  Future<List<GrnModel>> getGrnsForPo(int poId) async {
    final res = await _dio.get(ApiEndpoints.goodsReceivedNotes, queryParameters: {'by_po_id': poId});
    final data = res.data['data'] as Map<String, dynamic>? ?? {};
    final list = data['goods_received_notes'] as List? ?? [];
    return list.map((e) => GrnModel.fromJson(e as Map<String, dynamic>)).toList();
  }
    ...
    final body = {
      "grn_line_items": newLineItems.map((li) {
        return {
          ...
          "new_batches": li.receivedBatches.map((b) => {
            "batch_code": b.batchCode,
            "quantity": b.quantity,
            if (b.expiryDate != null && b.expiryDate!.isNotEmpty) "expiry_date": b.expiryDate,
            if ((b.manufactureDate ?? b.manufacturedDate) != null && (b.manufactureDate ?? b.manufacturedDate)!.isNotEmpty)
              "manufacturing_date": b.manufactureDate ?? b.manufacturedDate,
          }).toList(),
          "serial": li.receivedSerials,
        };
      }).toList(),
    };
    final res = await _dio.post(ApiEndpoints.saveGrnLineItems(grnId.toString()), data: body);
    if (res.data is Map<String, dynamic>) {
      final status = res.data['status']?.toString().toLowerCase();
      if (status != null && status != 'success' && status != 'ok') {
        throw Exception(res.data['message'] ?? res.data['error'] ?? 'Failed to update GRN line items');
      }
    }
  ...
  Future<GrnModel> updateGrnStatus(int grnId, String newStatus) async {
    ...
    if (res != null && res.data is Map<String, dynamic>) {
      final status = res.data['status']?.toString().toLowerCase();
      if (status != null && status != 'success' && status != 'ok') {
        throw Exception(res.data['message'] ?? res.data['error'] ?? 'Failed to update status');
      }
    }
    return await getGrnDetail(grnId);
  }
  ...
  Future<GrnModel> submitGrnQc(int grnId, List<GrnLineItemModel> qcLineItems) async {
    ...
    final res = await _dio.post(ApiEndpoints.saveQcLineItems(grnId.toString()), data: body);
    if (res.data is Map<String, dynamic>) {
      final status = res.data['status']?.toString().toLowerCase();
      if (status != null && status != 'success' && status != 'ok') {
        throw Exception(res.data['message'] ?? res.data['error'] ?? 'Failed to submit QC');
      }
    }
    return await getGrnDetail(grnId);
  }
  ...
  Future<List<PoSkuItemModel>> getPurchaseOrderSkuItems(int poId, [int? grnId]) async {
    ...
    return list.map((e) => PoSkuItemModel.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<bool> validateSerial(String serialNumber, [int productSkuId = 0]) async {
    try {
      final res = await _dio.get(ApiEndpoints.verifySerial(serialNumber, productSkuId.toString()));
  ...
  Future<bool> checkSerialExists(String serialNumber, [int productSkuId = 0]) async {
